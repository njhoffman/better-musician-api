...
const { getTables } = require('lib/utils/db');
const {
  seedModel,
  emptyModel,
  resetModel,
...
const AdminRoutes = (passport) => {
  ...
  const adminListAll = ({ _ctx }, res, next) => (
    ...
    Promise.map(Object.keys(allModels), (name) => (
      outputModel({
        name,
        context: _ctx,
  ...
  const adminResetAll = ({ _ctx, params: { group } }, res, next) => (
    Promise.mapSeries(Object.keys(allModels), (modelName) => (
      resetModel(modelName, group, _ctx, allModels)
    ))
      .then(output => outputData({ res, output, _ctx }))
      .catch(next)
  );

  const adminResetModel = ({ params: { model, group }, _ctx }, res, next) => (
    resetModel(model, group, _ctx, allModels)
      .then(output => outputData({ res, output, _ctx }))
      .catch(next)
  );
  ...
  router.get('/reset/all/:group',    handleRoute, adminResetAll);
  router.get('/reset/:model',        handleRoute, adminResetModel);
  router.get('/reset/:model/:group', handleRoute, adminResetModel);
