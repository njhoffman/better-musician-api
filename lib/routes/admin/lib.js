...
const { dumpTable } = require('lib/utils/db');
...
const outputModel = ({ name, context, model, deep }) => (
  (deep ? model.allDeep() : model.all())
    .then(records => {
      if (context === 'html') {
        return [
          '<div class="col">',
...
const resetModel = (name, group, context, models) => (
  models[name].reset(group)
    .then((results) => {
      if (results.errors === 0) {
