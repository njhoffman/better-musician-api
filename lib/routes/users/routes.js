...
const { log, warn } = require('debugger-256')('api:routes:user');
...
export default function (passport, models) {
  ...
  const registerUser = (req, res) => {
    setupAuth(req, res);
    log(`Registered user ${req.user.id}, email: ${req.user.email}`);
  ...
  const loginUser =  (req, res) => {
    setupAuth(req, res);
    log(`User logged in: ${req.user.id},  email: %${req.user.email}%`, { color: 'cyan' });
    res.json({
      data: req.user,
      status: 200
    });
    ...
    User.modelByField({ email: req.user.email })
      .then(user => {
        if (!user) {
          ...
          log('User validated', user.cleanFields);
          return res.json({
            status: 200,
