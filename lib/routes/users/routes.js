...
const UserRoutes = (passport, respondSuccess, respondError) => {
  ...
  const registerFail = (err, req, res, next) => {
    ...
    if (status === 401) {
      ...
      err.message = `Registration failure: ${status} ${msg}`;
      err.stack = [
        new Error().stack
          .split('\n')
          .slice(0, 2)
  ...
  const showMeDeep = (req, res, next) => {
    const { _models_: { User }, user: { id } } = req;
    User.modelById(id)
      .then(foundUser => foundUser.deep())
      .then(deepUser => respondSuccess('query', { records: deepUser })(req, res, next))
      .catch(next);
  };
  ...
  router.get('/me/deep',           handleRoute, showMeDeep);
