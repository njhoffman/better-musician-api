...
const { log, info, trace } = require('debugger-256')('api:routes:songs');
...
export default function (passport, models) {
  ...
  const fetchSongs = (req, res) => {
    ...
    let tables = {};
    log(`Fetch songs for user %${req.user.email}% (${id})`, { color: 'bold' });
    return Song.findByField({ user: id })
      .then(songs => {
        ...
        trace('Fetch songs Results', tables);
        res.json({
          status: 200,
          data: {
            tables: tables
