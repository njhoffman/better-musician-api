...
const logger = (subsystem, extra = {}) => {
  ...
  loggerInstance = createLogger(loggerName, childParams, extra);
  loggerInstance.child = (args) => logger(args.subsystem || subsystem, _.merge(extra, args));
  loggerInstance.debug(
    { ...(_.omit(extra, 'subsystem')) },
    `loaded logger configuration for ${loggerName} (${subsystem})`
  );
  return loggerInstance;
};
...
module.exports.initLogger = (name = loggerName) => (subsystem, extra) => createLogger(name, { subsystem }, extra);
