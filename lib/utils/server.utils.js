const { omit, isEmpty, isObject, keys } = require('lodash');
...
const requestOutput = (req, res, next) => {
  const { log, info, trace } = require('./logger')('api:request');
  ...
  if (keys(query).length > 0 && keys(query).length !== keys(body).length) { info('Query', query); }
...
const morganOutput = (tokens, req, res, next) => {
  const { log, info, trace } = require('./logger')('api:response');
  ...
  trace('Headers', (res.headers ? res.headers : req.headers));
