...
const initRequestMetadata = (req, res, next) => {
  ...
  const { user = {} } = req;
  const userDesc = _.has(req, 'headers.uid') ? `${req.headers.uid}` : user.email || '';
  ...
  req._metadata_ = {
    ..._metadata_,
    loggerFields: () => ({
      _requestId:        _metadata_.id,
      _requestIp:        _metadata_.ip,
      _requestLocation:  _metadata_.location,
      _requestUser:      _metadata_.user,
...
const initModelMetadata = (models) => (req, res, next) => {
  ...
  Object.keys(models).forEach(modelKey => models[modelKey].initMetadata(_metadata_));
  req._models_ = models;
  next();
};
