...
const { debug, info, trace, error } = require('./logger')('api:db');
...
const dumpModels = (models) => {
  ...
  Object.keys(models).forEach(name => {
    ...
    toDump.push(models[name].all.then(data => trace({ data }, `Dumping ${name}:`)));
    toDump.push(models[name].count.then(count => info({ color: 'bold'}, `Model: %${name}% (${count} records)`)));
  });
