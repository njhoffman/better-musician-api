  ...
  User.findById(payload.id)
    .then(foundUser => {
      if (_.isObject(foundUser)) {
        ...
        req.logger = req.logger.child({ _requestUser: foundUser.email, subsystem: 'passport:strategies' });
        req.logger.info(`JWT authentication success for user: ${foundUser.email}`);
...
const loginStrategy = (req, email, password, done) => {
  ...
  User.modelByField({ email })
    .then(foundUser => {
      ...
      info({ _trace: { userFields: foundUser.cleanFields } },
      ...
      req.logger = req.logger.child({ _requestUser: foundUser.email, subsystem: 'passport:strategies' });
      req.logger.info({ _trace: foundUser.cleanFields }, `Login authentication success for user: ${foundUser.email}`);
