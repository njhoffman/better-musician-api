...
class User extends BaseModel {
  ...
  // seedSongs(num) {
  //   const { Song, Field } = getModels();
  //   const fieldData = _.map(Field.seedData, sd => (
  //     ({ ...sd, user: this.fields.id })
  //   ));
  //   const songData = _.map(Song.seedData, sd => (
  //     ({ ...sd, user: this.fields.id })
  //   ));
  //   Field.seed(0, songData).then(({ generated_keys }) => {
  //     console.log('field res', generated_keys);
  //     Song.seed(num, songData);
  //   });
  // }
