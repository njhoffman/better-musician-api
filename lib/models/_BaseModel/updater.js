const { get, pick, isObject, pickBy, keys, has } = require('lodash');
...
const _logResult = function _logResult(results, newFields) {
  const { type, inserted, replaced, skipped, unchanged, changed = [] }  = results;
  ...
  const logMessage = [
    ...
    `${skipped || unchanged ? ') ' : ''}`,
    `${changed.length} total field changes, `,
    `record(s) during ${type} operation for model %${this.modelName}%`
  ].filter(Boolean).join(' ');

  this.logger.debug({
    _trace: { fields: this.fields, data: newFields, changed: this.changed },
    _dbOp: {
...
const _create = function _create(fieldsToAdd) {
  ...
  // const toAdd = { ...this.fields, createdAt: new Date().getTime() };
  const toAdd = { ...fieldsToAdd, createdAt: new Date().getTime() };
  this.logger.debug(
    { _debug: toAdd },
...
const _processIncomingFields = function _processIncomingFields(newFields) {
  ...
  return Promise.mapSeries(foreignModels, fModel => (
    Promise.all([fModel.save(), fModel.parentField])
  ))
    .then(fKeySaves => {
...
const Save = function Save(newFields) {
  ...
  if (newFields) {
    this.rawFields = newFields;
    this.validatedFields = this.populateDefaults(newFields);
  }
