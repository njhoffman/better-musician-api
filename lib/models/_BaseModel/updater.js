const { pick, omit, pickBy, keys, has } = require('lodash');
...
const _add = function _add() {
  ...
  return this.db
    ...
    .run(this.conn)
    .then((results) => {
      const { generated_keys: [id], changes, errors } = results;
      this.status.creating = false;
      if (errors && errors.length > 0) {
        throw new Error(`ERROR Adding Record: ${results.first_error}`);
...
const _logResult = function _logResult(results, newFields) {
  const { type, inserted, replaced, skipped, unchanged }  = results;
  ...
  const logMessage = [
    ...
    `${skipped || unchanged ? ') ' : ''}`,
    `${this.changed.length} total field changes, `,
  ...
  this.logger.debug({
    ...
    _dbOp: {
      ...
      operation: type,
      table: this.tableName,
      resultCount: this.changed.length,
