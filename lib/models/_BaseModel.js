const { omit, isEmpty, pickBy, has, keys, isArray, isObject, merge }  = require('lodash');
...
class BaseModel {
  ...
  static seed(num = 0, seedData = this.seedData) {
    ...
    return this.db
      ...
      .then(results => {
        ...
        this.changed = this.changedFields(results.changes);
        if (results.errors === 0) {
          this.logger.debug({
          ...
          return { ...omit(results, 'changes'), changed: this.changed };
        }
  ...
  static delete(uniqueFields) {
    // unlike instantiated delete this can remove multiple fields
    this.status.deleting = true;
    return this.db
      .table(this.tableName)
      .filter(uniqueFields)
      ...
      .then(results => {
        ...
        if (results.errors === 0) {
          ...
          }, `Deleted ${results.deleted} records for model ${this.modelName}`);

          return { ...omit(results, 'changes'), changed: this.changed };
        }
        throw new Error(`ERROR deleting ${this.modelName}: ${results.first_error}`);
      });
  }
  ...
  constructor({ tableName, tableKeys, modelName }, data = null, skipValidation = false) {
    ...
    this.changedFields = this.constructor.changedFields;
    this.changed = { old: [], new: [], delta: [] };
  }
  ...
  set cleanFields(fieldData) {
    if (!fieldData || !isObject(fieldData) || keys(fieldData).length === 0) {
      return;
    }
