...
const { trace, log, info, error } = require('debugger-256')('api:baseModel');
...
class BaseModel {
  ...
  static generate (num = 0, mergeObj) {
    ...
    info(`Generating records for table: ${this.tableName}, model: ${this.modelName}`);
    return this.db
      .table(this.tableName)
      .insert(exampleData)
      .run(this.conn)
      .then(results => {
        log(`Generated ${results.inserted} records for model ${this.modelName}`);
        return results;
      });
  }
