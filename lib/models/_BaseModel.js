const {
  ...
  isEmpty, pickBy, has, keys, isArray
}  = require('lodash');
...
class BaseModel {
  ...
  constructor({ tableName, tableKeys, modelName }, data = null, recordExists) {
    ...
    if (recordExists) {
      ...
      this.rawFields = fieldData;
      this.validatedFields = this.populateDefaults(fieldData);
    }
  }
  ...
  set validatedFields(fields) { this._validatedFields = this.validateFields(fields); }
  ...
  set rawFields(fields) { this._rawFields = fields; }
