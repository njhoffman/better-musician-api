...
const { AuthLockError } = require('../Errors');
const { getDbModule } = require('../utils/db');
const baseLogger = require('../utils/logger')('model:base');

const validator = require('./ModelBase/validator');
const seed = require('./ModelBase/seed');
const updater = require('./ModelBase/updater');
const queries = require('./ModelBase/queries');
...
class ModelBase {
  ...
  static get logger() {
    ...
    return baseLogger.child({ subsystem: 'modelBase', ...meta });
  }
  ...
  static reset(seedGroup) {
    return this.empty()
      .then(() => this.seed(0, seedGroup));
  }
  ...
  static testUnique(fields) {
    const uniqueFields = pickBy(this.tableKeys, (tk, name) => (tk.unique));
    ...
    if (has(fields, 'id') && !isEmpty(fields.id)) {
  ...
  set existingFields(fields) {
    // should only be set within ModelBase when field updated or constructed with existing record
    this._existingFields = fields;
    this._fields = fields;
  }

  get existingFields() { return this._existingFields; }
...
ModelBase.prototype.validateFields = validator;
ModelBase.prototype.save = updater;
Object.assign(ModelBase, seed);
Object.assign(ModelBase, queries);
// ModelBase.all = queries.all;
